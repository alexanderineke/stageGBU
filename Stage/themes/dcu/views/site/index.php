...
<?php if(!empty($collection)): ?>
    <?php $cs = Yii::app()->getClientScript(); $cs->registerScript("collections-spotlight", "jQuery('#collections-spotlight').carousel();"); ?>
                ...
                <?php endif; ?>
                        <article class="span4 collection-thumb">
                            <?php if($subcollection->thumb): ?>
                                <?php echo CHtml::image('uploads/afbeeldingen/'.$subcollection->thumb->location.'/thumb/'.$subcollection->thumb->file.$subcollection->thumb->format, $subcollection->title, array('class'=>'img-polaroid collection-thumb-img span5')); ?>
                            <?php endif; ?>
                            <a href="<?php echo Yii::app()->createUrl("collection/view",array("id"=>$subcollection->id)); ?>"  class="span7">
    ...
    <div class="span6">
        ...
        if(!empty($popularTags)){
            foreach($popularTags as $tag) {
                echo '<a class="btn btn-primary tag" href="'.Yii::app()->baseUrl.'/index.php?r=search/tag&tag='.$tag['slug'].'">'.$tag['name'].'</a>';
            }
        }
        ?>
    </div>
