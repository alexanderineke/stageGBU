 ...
 *
 * The followings are the available columns in table '{{audio_tag}}':
 * @property integer $id
 * @property integer $audio_id
 * @property integer $tag_id
 * @property integer $state
 */
class AudioTag extends CActiveRecord
{
    /**
     * Returns the static model of the specified AR class.
     * @param string $className active record class name.
     * @return AudioTag the static model class
     */
    ...
    public function rules()
    {
        ...
        return array(
            array('audio_id, tag_id', 'required'),
            array('audio_id, tag_id, state', 'numerical', 'integerOnly'=>true),
            // The following rule is used by search().
            // Please remove those attributes that should not be searched.
            array('id, audio_id, tag_id, state', 'safe', 'on'=>'search'),
        );
    }
    ...
    public function add($audio_id, $tagIds){
        ...
        foreach ($notInDB as $i) {
            $sql = "insert into tbl_audio_tag (audio_id, tag_id, state) values (:audio_id, :tag_id, 1)";
            $parameters = array(":audio_id"=>$audio_id,
                                ":tag_id"=>$i);
    ...
    public function check($audio_id, $tagIds){
        ...
        $criteria->condition = 'audio_id='.$audio_id.' AND state=1';
    ...
    public function getTag($audio_id){
        $criteria=new CDbCriteria;
        $criteria->select = array('tag_id');
        $criteria->condition = 'audio_id='.$audio_id.' AND state=1';
        $criteria->limit = 1;
