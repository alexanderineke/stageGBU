...
$this->widget('bootstrap.widgets.TbButton', array(
    'htmlOptions' => array('id'=>'selector-button', 'data-toggle'=>'false'), 
...
$gridDataProvider = new CArrayDataProvider(array(
    array('id'=>0, 'type'=>'', 'title'=>'Geen selectie'),
));
...
$this->widget('bootstrap.widgets.TbGridView', array(
    ...
    'template'=>'{items}',
    'columns'=>array(
        array('name'=>'type', 'type'=>'raw', 'header'=>'Type', 'htmlOptions'=>array('width'=>'15%')),
        array('name'=>'title', 'header'=>'Naam'),
        array('name'=>'action', 'header'=>'Verwijder')
    ),
    'htmlOptions'=>array(
        'style'=>'display:none;',
        'id'=>'selection-widget'
...
if ($imageSearch->totalItemCount != 0) {
    ...
    $cs = Yii::app()->getClientScript(); $cs->registerScript('image-grid', 'Grid.init();');
...
if (($audioSearch->totalItemCount != 0 && !Yii::app()->request->isAjaxRequest) || Yii::app()->request->isAjaxRequest) {
    $results[] = array(
        ...
        'content' => $this->widget('bootstrap.widgets.TbGridView', array(
            ...
            'filter'=>$audioModel,
            'columns'=>array(
                array('name'=>'title', 'header'=>'Naam document', 'value'=>'CHtml::link($data->title, Yii::app()->createUrl("audio/view",array("id"=>$data->id)))', 'type'=>'raw', 'filter'=>CHtml::activeTextField($audioModel, 'title', array('placeholder'=>'Zoek op titel..'))),
                array('name'=>'tag_search', 'header'=>'Steekwoorden', 'value'=>'objectToTagStringAudio($data->id)', 'filter'=>CHtml::activeTextField($audioModel, 'tag_search', array('placeholder'=>'Zoek op steekwoord..'))), //Omdat een result meerdere tags kan hebben moeten we deze verwerken.
                array('name'=>'created_on', 'header'=>'Toegevoegd op', 'value'=>'($data->created_on !== "0000-00-00 00:00:00" ? strftime("%e %B %Y", strtotime($data->created_on)) : "Datum niet beschikbaar")', 'filter'=>''),
            ),
            'enableHistory'=>true,
            'pager' => array('class' => 'bootstrap.widgets.TbPager', 'prevPageLabel' => '&laquo;', 'nextPageLabel' => '&raquo;'),
