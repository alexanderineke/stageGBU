...
/**
	...
	**/
	...
	{
		$model = new AudioTemp;
        $uploadedFile = CUploadedFile::getInstanceByName('Audio[file]');
        $rnd = rand(0,9999); 
...
/**
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Audio'])){	
			...
			$fileQueue = Yii::app()->user->getState('filesToProcess');
			if($fileQueue){
				$audioTempModel = AudioTemp::model()->findByPk($fileQueue[0]);
				$file = $audioTempModel->getAttributes(array('file', 'format', 'location'));
			}
			...
			if($this->generateTags()){ //Kijk welke tags er niet zijn en welke niet
				...
				if($model->save()){ //Probeer het formulier op te slaan 
					...
					if($this->saveTags($model->id)){ //Probeer de tags te koppelen
						if($fileQueue){
							if(AudioFile::model()->saveAudio($model->id, $this->tags[0], $file)){ //Kijk of het betand koppelen lukt
			...
			if($this->generateTags()){ //Kijk welke tags er niet zijn en welke niet
				...
				if($model->save()){ //Probeer het formulier op te slaan 
					...
					if($this->saveTags($model->id)){ //Probeer de tags te koppelen
						...
						// if(isset($_POST['Audio']['collection'])){ //Kijk of er een collectie is geselecteerd
						// 	$collection = (int)$_POST['Audio']['collection'];
						// 	if($collection > 0){
						// 		if(!CollectionAudio::add($model->id, $collection)) //Kijk of het toevoegen aan de collectie is gelukt
						// 			Yii::app()->user->setFlash('warning', "Het audio bestand kon niet aan de door u geselecteerde collectie worden toegevoegd.");		
						// 	}
						// }
						if(!$model->audios){
							if(AudioFile::model()->saveAudio($model->id, $this->tags[0], $file)){ //Kijk of het betand koppelen lukt
